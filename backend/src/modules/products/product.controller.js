...
export const createProduct = async (req, res) => {
  try {
    ...
    if (mainImageFile) {
      ...
      mainImagePublicUrl = supabase.storage.from(SUPABASE_BUCKET_PRODUCT).getPublicUrl(filePath)
        .data.publicUrl;
    }
    ...
    for (const file of imagesFiles) {
      ...
      imagesPublicUrls.push(
        supabase.storage.from(SUPABASE_BUCKET_PRODUCT).getPublicUrl(filePath).data.publicUrl
      );
    }
